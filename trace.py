...
def catch_trace_reply(s, ID, time_sent, timeout=1):
    # create while loop
    while True:
...
def single_trace_request(s, addr=None):
    ...
    # Create ICMP Packet
    packet1 = IPPacket(dst = addr,idf=pkt_id,ttl = 1).raw
    packet2 = IPPacket(dst = addr,idf=pkt_id,ttl = 2).raw
    # Send ICMP Packet
    while packet1:
        sent = s.sendto(packet1, (addr, 1))
...
def main():
    ...
    print('TRACE {0} ({1}) 56(84) bytes of data.'.format(addr, socket.gethostbyname(addr)))
    # Request sent
